...
用法:
    ...
    python collect_native_incr.py --shop "光明邨大酒家(淮海路店)" --org-code "0801" --scroll 10
    python collect_native_incr.py --shop "光明邨大酒家(淮海路店)" --org-code "0801" --no-incr   # 禁用增量停止,全量采集

配置文件:
    dianping_config.json 存放环境相关参数(adb路径/输出目录/数据库等),
...
def main():
    ...
    if db_sync is not None and not args.no_incr:
        ...
        print("  [增量] 已禁用(--no-incr 或 --no-db),将全量采集")
    ...
    while True:
        ...
        # [增量] 停止条件:本屏出现日期 ≤ 边界日期的评论 → 已达增量采集边界,结束。
        # 上面的落库/CSV/JSON 已保存,直接退出,不再向下滑采更早的旧评论。
        # 用 _normalize_date 归一化比较(库里 review_date 为 YYYY-MM-DD)。
        # 采用 ≤ 而非 =:当边界当天恰好无评论(自然空档或滑动跳过)时,继续等一个
        # 严格相等的边界日期会越过边界往下过采全量旧评论;≤ 则遇到第一条在边界
        # 当天或更早的评论即停,更稳健(库里已有更早记录靠 hash 去重,安全)。
        if stop_date:
            boundary_hit = any(
                CSVExporter._normalize_date((c.get("date") or "").strip()) <= stop_date
                for c in valid_cards
            )
            if boundary_hit:
                print(f"  [增量] 命中边界日期 {stop_date},已达增量采集边界,停止采集")
                break
